...
TEST(Mtproto, GetHostByNameActor) {
  ...
  td::vector<td::ActorOwn<td::GetHostByNameActor>> actors;
  {
    ...
    auto run = [&](td::ActorId<td::GetHostByNameActor> actor_id,
                   td::string host, bool prefer_ipv6, bool allow_ok,
                   bool allow_error) {
      auto promise = td::PromiseCreator::lambda(
          [&cnt, &actors, num = cnt, host, allow_ok,
           allow_error](td::Result<td::IPAddress> r_ip_address) {
            if (r_ip_address.is_error() && !allow_error) {
              LOG(ERROR) << num << " \"" << host << "\" "
                         << r_ip_address.error();
            }
            if (r_ip_address.is_ok() && !allow_ok &&
                (r_ip_address.ok().is_ipv6() ||
                 r_ip_address.ok().get_ipv4() != 0)) {
      ...
      cnt++;
      td::send_closure_later(actor_id, &td::GetHostByNameActor::run, host, 443,
                             prefer_ipv6, std::move(promise));
    };

    td::vector<td::string> hosts = {
        "127.0.0.2",
        "1.1.1.1",
        "localhost",
        "web.telegram.org",
    ...
    for (const auto &types :
         {td::vector<td::GetHostByNameActor::ResolverType>{
      ...
      auto actor = td::create_actor<td::GetHostByNameActor>(
          "GetHostByNameActor", std::move(options));
      ...
      for (auto host : hosts) {
        for (auto prefer_ipv6 : {false, true}) {
          ...
          bool allow_both = host == "127.0.0.1." || host == "localhost" ||
                            (host == "москва.рф" && prefer_ipv6);
...
TEST(Time, to_unix_time) {
  ...
  ASSERT_EQ(60 * 60 + 60 + 1,
            td::HttpDate::to_unix_time(1970, 1, 1, 1, 1, 1).move_as_ok());
  ASSERT_EQ(24 * 60 * 60,
            td::HttpDate::to_unix_time(1970, 1, 2, 0, 0, 0).move_as_ok());
  ASSERT_EQ(31 * 24 * 60 * 60,
            td::HttpDate::to_unix_time(1970, 2, 1, 0, 0, 0).move_as_ok());
  ASSERT_EQ(365 * 24 * 60 * 60,
            td::HttpDate::to_unix_time(1971, 1, 1, 0, 0, 0).move_as_ok());
  ASSERT_EQ(1562780559,
            td::HttpDate::to_unix_time(2019, 7, 10, 17, 42, 39).move_as_ok());
}

TEST(Time, parse_http_date) {
  ASSERT_EQ(784887151,
            td::HttpDate::parse_http_date("Tue, 15 Nov 1994 08:12:31 GMT")
                .move_as_ok());
}
...
TEST(Mtproto, encrypted_config) {
  td::string data =
      "   hO//tt "
      "\b\n\tiwPVovorKtIYtQ8y2ik7CqfJiJ4pJOCLRa4fBmNPixuRPXnBFF/"
      "3mTAAZoSyHq4SNylGHz0Cv1/"
      "FnWWdEV+BPJeOTk+ARHcNkuJBt0CqnfcVCoDOpKqGyq0U31s2MOpQvHgAG+"
      "Tlpg02syuH0E4dCGRw5CbJPARiynteb9y5fT5x/"
      "kmdp6BMR5tWQSQF0liH16zLh8BDSIdiMsikdcwnAvBwdNhRqQBqGx9MTh62MDmlebjtczE9G"
      "z0z5cscUO2yhzGdphgIy6SP+"
      "bwaqLWYF0XdPGjKLMUEJW+rou6fbL1t/EUXPtU0XmQAnO0Fh86h+AqDMOe30N4qKrPQ==  "
      " ";
  td::telegram_api::object_ptr<td::telegram_api::help_configSimple> config =
      td::decode_config(data).move_as_ok();
}

class TestPingActor final : public td::Actor {
 public:
  TestPingActor(td::IPAddress ip_address, td::Status *result)
      : ip_address_(ip_address), result_(result) {}
 ...
 private:
  ...
  void start_up() final {
    ...
    ping_connection_ = td::mtproto::PingConnection::create_req_pq(
        td::mtproto::RawConnection::create(
            ...
            td::mtproto::TransportType{td::mtproto::TransportType::Tcp, 0,
                                       td::mtproto::ProxySecret()},
            nullptr),
        3);

    td::Scheduler::subscribe(
        ping_connection_->get_poll_info().extract_pollable_fd(this));
    is_inited_ = true;
    set_timeout_in(10);
    yield();
  }
  ...
  void tear_down() final {
    if (is_inited_) {
      td::Scheduler::unsubscribe_before_close(
          ping_connection_->get_poll_info().get_pollable_fd_ref());
    }
    td::Scheduler::instance()->finish();
  }
...
static td::int32 get_default_dc_id() { return 10002; }
...
class Mtproto_ping final : public td::Test {
 public:
  ...
  bool step() final {
    if (!is_inited_) {
      sched_
          .create_actor_unsafe<TestPingActor>(
              0, "Pinger", get_default_ip_address(), &result_)
          .release();
      sched_.start();
      is_inited_ = true;
    }
...
class HandshakeContext final : public td::mtproto::AuthKeyHandshakeContext {
 public:
  td::mtproto::DhCallback *get_dh_callback() final { return nullptr; }
 ...
 private:
  std::shared_ptr<td::mtproto::PublicRsaKeyInterface> public_rsa_key_ =
      td::PublicRsaKeySharedMain::create(true);
};

class HandshakeTestActor final : public td::Actor {
 public:
  HandshakeTestActor(td::int32 dc_id, td::Status *result)
      : dc_id_(dc_id), result_(result) {}
 ...
 private:
  ...
  void loop() final {
    if (!wait_for_raw_connection_ && !raw_connection_) {
      ...
      if (r_socket.is_error()) {
        finish(td::Status::Error(PSTRING() << "Failed to open socket: "
                                           << r_socket.error()));
        return stop();
      }
    ...
    if (raw_connection_ && handshake_) {
      ...
      td::create_actor<td::mtproto::HandshakeActor>(
          "HandshakeActor", std::move(handshake_), std::move(raw_connection_),
          td::make_unique<HandshakeContext>(), 10.0,
          td::PromiseCreator::lambda(
              [actor_id = actor_id(this)](
                  td::Result<td::unique_ptr<td::mtproto::RawConnection>>
                      raw_connection) {
                td::send_closure(actor_id, &HandshakeTestActor::on_connection,
                                 std::move(raw_connection), 1);
              }),
          td::PromiseCreator::lambda(
              [actor_id = actor_id(this)](
                  td::Result<td::unique_ptr<td::mtproto::AuthKeyHandshake>>
                      handshake) {
                td::send_closure(actor_id, &HandshakeTestActor::on_handshake,
                                 std::move(handshake), 1);
              }))
          .release();
  ...
  void on_connection(
      td::Result<td::unique_ptr<td::mtproto::RawConnection>> r_raw_connection,
      bool dummy) {
  ...
  void on_handshake(
      td::Result<td::unique_ptr<td::mtproto::AuthKeyHandshake>> r_handshake,
      bool dummy) {
...
class Mtproto_handshake final : public td::Test {
 public:
  ...
  bool step() final {
    if (!is_inited_) {
      sched_
          .create_actor_unsafe<HandshakeTestActor>(
              0, "HandshakeTestActor", get_default_dc_id(), &result_)
          .release();
      sched_.start();
      is_inited_ = true;
    }
...
class Socks5TestActor final : public td::Actor {
 public:
  void start_up() final {
    auto promise = td::PromiseCreator::lambda(
        [actor_id =
             actor_id(this)](td::Result<td::BufferedFd<td::SocketFd>> res) {
          td::send_closure(actor_id, &Socks5TestActor::on_result,
                           std::move(res), false);
        });

    class Callback final : public td::TransparentProxy::Callback {
     public:
      explicit Callback(td::Promise<td::BufferedFd<td::SocketFd>> promise)
    ...
    if (r_socket.is_error()) {
      return promise.set_error(td::Status::Error(
          PSTRING() << "Failed to open socket: " << r_socket.error()));
    }
    td::create_actor<td::Socks5>(
        "Socks5", r_socket.move_as_ok(), mtproto_ip_address, "", "",
...
TEST(Mtproto, notifications) {
  td::vector<td::string> pushes = {
      "eyJwIjoiSkRnQ3NMRWxEaWhyVWRRN1pYM3J1WVU4TlRBMFhMb0N6UWRNdzJ1cWlqMkdRbVR1"
      "WXVvYXhUeFJHaG1QQm8yVElYZFBzX2N3b2RIb3lY"
      "b2drVjM1dVl0UzdWeElNX1FNMDRKMG1mV3ZZWm4zbEtaVlJ0aFVBNGhYUWlaN0pfWDMyZDBL"
      "QUlEOWgzRnZwRjNXUFRHQWRaVkdFYzg3bnFPZ3hD"
      "NUNMRkM2SU9fZmVqcEpaV2RDRlhBWWpwc1k2aktrbVNRdFZ1MzE5ZW04UFVieXZudFpfdTNu"
      "d2hjQ0czMk96TGp4S1kyS1lzU21JZm1GMzRmTmw1"
      "QUxaa2JvY2s2cE5rZEdrak9qYmRLckJyU0ZtWU8tQ0FsRE10dEplZFFnY1U5bVJQdU80b1d2"
      "NG5sb1VXS19zSlNTaXdIWEZyb1pWTnZTeFJ0Z1dN"
      "ZyJ9",
      "eyJwIjoiSkRnQ3NMRWxEaWlZby1GRWJndk9WaTFkUFdPVmZndzBBWHYwTWNzWDFhWEtNZC03"
      "T1Q2WWNfT0taRURHZDJsZ0h0WkhMSllyVG50RE95"
      "TkY1aXJRQlZ4UUFLQlRBekhPTGZIS3BhQXdoaWd5b3NQd0piWnJVV2xRWmh4eEozUFUzZjBN"
      "RTEwX0xNT0pFN0xsVUFaY2dabUNaX2V1QmNPZWNK"
      "VERxRkpIRGZjN2pBOWNrcFkyNmJRT2dPUEhCeHlEMUVrNVdQcFpLTnlBODVuYzQ1eHFPdERq"
      "cU5aVmFLU3pKb2VIcXBQMnJqR29kN2M5YkxsdGd5"
      "Q0NGd2NBU3dJeDc3QWNWVXY1UnVZIn0"};
  td::string key =
      "uBa5yu01a-nJJeqsR3yeqMs6fJLYXjecYzFcvS6jIwS3nefBIr95LWrTm-"
      "IbRBNDLrkISz1Sv0KYpDzhU8WFRk1D0V_"
      "qyO7XsbDPyrYxRBpGxofJUINSjb1uCxoSdoh1_"
      "F0UXEA2fWWKKVxL0DKUQssZfbVj3AbRglsWpH-jDK1oc6eBydRiS3i4j-"
      "H0yJkEMoKRgaF9NaYI4u26oIQ-Ez46kTVU-R7e3acdofOJKm7HIKan_5ZMg82Dvec2M6vc_"
      "I54Vs28iBx8IbBO1y5z9WSScgW3JCvFFKP2MXIu7Jow5-"
      "cpUx6jXdzwRUb9RDApwAFKi45zpv8eb3uPCDAmIQ";
  td::vector<td::string> decrypted_payloads = {
      "eyJsb2Nfa2V5IjoiTUVTU0FHRV9URVhUIiwibG9jX2FyZ3MiOlsiQXJzZW55IFNtaXJub3Yi"
      "LCJhYmNkZWZnIl0sImN1c3RvbSI6eyJtc2dfaWQi"
      "OiI1OTAwNDciLCJmcm9tX2lkIjoiNjI4MTQifSwiYmFkZ2UiOiI0MDkifQ",
      "eyJsb2Nfa2V5IjoiIiwibG9jX2FyZ3MiOltdLCJjdXN0b20iOnsiY2hhbm5lbF9pZCI6IjEx"
      "NzY4OTU0OTciLCJtYXhfaWQiOiIxMzU5In0sImJh"
      "ZGdlIjoiMCJ9"};
  key = td::base64url_decode(key).move_as_ok();

  for (size_t i = 0; i < pushes.size(); i++) {
    auto push = td::base64url_decode(pushes[i]).move_as_ok();
    auto decrypted_payload =
        td::base64url_decode(decrypted_payloads[i]).move_as_ok();

    auto key_id = td::mtproto::DhHandshake::calc_key_id(key);
    ASSERT_EQ(key_id, td::NotificationManager::get_push_receiver_id(push).ok());
    ASSERT_EQ(decrypted_payload,
              td::NotificationManager::decrypt_push(key_id, key, push).ok());
  }
}
...
class FastPingTestActor final : public td::Actor {
 ...
 private:
  ...
  void start_up() final {
    ...
    auto r_socket = td::SocketFd::open(ip_address);
    if (r_socket.is_error()) {
      *result_ = td::Status::Error(PSTRING() << "Failed to open socket: "
                                             << r_socket.error());
      return stop();
    }
    ...
    auto raw_connection = td::mtproto::RawConnection::create(
        ...
        td::mtproto::TransportType{td::mtproto::TransportType::Tcp, 0,
                                   td::mtproto::ProxySecret()},
        nullptr);
    auto handshake = td::make_unique<td::mtproto::AuthKeyHandshake>(
        get_default_dc_id(), 60 * 100 /*temp*/);
    td::create_actor<td::mtproto::HandshakeActor>(
        ...
        td::PromiseCreator::lambda(
            [actor_id = actor_id(this)](
                td::Result<td::unique_ptr<td::mtproto::RawConnection>>
                    raw_connection) {
              td::send_closure(actor_id, &FastPingTestActor::on_connection,
                               std::move(raw_connection), 1);
            }),
        td::PromiseCreator::lambda(
            [actor_id = actor_id(this)](
                td::Result<td::unique_ptr<td::mtproto::AuthKeyHandshake>>
                    handshake) {
              td::send_closure(actor_id, &FastPingTestActor::on_handshake,
                               std::move(handshake), 1);
            }))
        .release();
  }
  ...
  void on_raw_connection(
      td::Result<td::unique_ptr<td::mtproto::RawConnection>> r_connection) {
  ...
  void loop() final {
    if (handshake_ && connection_) {
      ...
      if (iteration_ % 2 == 0) {
        ...
        auth_data->reset_server_time_difference(
            handshake_->get_server_time_diff());
        auth_data->set_server_salt(handshake_->get_server_salt(),
                                   td::Time::now());
        auth_data->set_future_salts({td::mtproto::ServerSalt{0u, 1e20, 1e30}},
                                    td::Time::now());
        auth_data->set_use_pfs(true);
        td::uint64 session_id = 0;
        do {
          td::Random::secure_bytes(reinterpret_cast<td::uint8 *>(&session_id),
                                   sizeof(session_id));
        } while (session_id == 0);
      ...
      fast_ping_ = create_ping_actor(
          ...
          td::PromiseCreator::lambda(
              [actor_id = actor_id(this)](
                  td::Result<td::unique_ptr<td::mtproto::RawConnection>>
                      r_raw_connection) {
                td::send_closure(actor_id,
                                 &FastPingTestActor::on_raw_connection,
  ...
  void tear_down() final { td::Scheduler::instance()->finish(); }
};
...
class Mtproto_FastPing final : public td::Test {
 public:
  ...
  bool step() final {
    if (!is_inited_) {
      sched_
          .create_actor_unsafe<FastPingTestActor>(0, "FastPingTestActor",
                                                  &result_)
          .release();
      sched_.start();
      is_inited_ = true;
    ...
    class RunTest final : public td::Actor {
      void start_up() final {
        class Callback final : public td::TransparentProxy::Callback {
         public:
          void set_result(
              td::Result<td::BufferedFd<td::SocketFd>> result) final {
            if (result.is_ok()) {
              LOG(ERROR)
                  << "Unexpectedly succeeded to connect to MTProto proxy";
        ...
        td::create_actor<td::mtproto::TlsInit>(
            "TlsInit", r_socket.move_as_ok(), domain, "0123456789secret",
...
TEST(Mtproto, RSA) {
  ...
  ASSERT_EQ("U2nJEtB2AgpHrm3HB0yhpTQgb0wbesi9Pv/W1v/vULU=",
